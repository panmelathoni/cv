...
config =
    ...
    whatsNewPath: path.join(__dirname, 'whats-new.html')
...
config.amazonBucketProdPluginJson="craft-assets.invisionapp.com/craft/channels/production/production.json"
...
envSet = (key, value) ->
  ...
  if useEnvman
    ...
    execSync("echo `export #{key}='#{value}'`")
    console.log(cmd)
...
gulp.task 'generate-appcast', ->
    ...
    gulp.src(config.appCastPath)
        .pipe(template({
            ...
            releaseNotes: "https://support.invisionapp.com/hc/en-us/articles/215206763-Release-Notes-Craft"
        }))
...
gulp.task 'upload-to-s3', ->
    ...
    execSync("aws s3 cp #{config.whatsNewPath} s3://#{config.amazonBucketNext}whats-new.html --acl public-read", { stdio: [0, 1, 2] })
...
gulp.task 'backup-s3-version', ->
    ...
    getVersion uri, (err, buildNumber, versionNumber) ->
        ...
        execSync("aws s3 cp #{fromPath}whats-new.html #{destPath}whats-new.html", { stdio: [0, 1, 2] })

gulp.task 'backup-s3-plugins-on-release', ->
    if process.argv.indexOf("--nobackup") >- 1
        console.log "No backup"
        return
    ...
    getVersion uri, (err, buildNumber, versionNumber) ->
        if err
            throw err
        destPath="s3://#{config.amazonBucket}versions/#{versionNumber}-#{buildNumber}/"
        execSync("aws s3 cp s3://#{config.amazonBucketProdPluginJson} #{destPath}plugins-on-release.json --acl public-read", { stdio: [0, 1, 2] })

gulp.task 'backup-s3-plugins-on-retire', ->
    if process.argv.indexOf("--nobackup") >- 1
        console.log "No backup"
        return
    ...
    getVersion uri, (err, buildNumber, versionNumber) ->
        if err
            throw err
        destPath="s3://#{config.amazonBucket}versions/#{versionNumber}-#{buildNumber}/"
        execSync("aws s3 cp s3://#{config.amazonBucketProdPluginJson} #{destPath}plugins-on-retire.json --acl public-read", { stdio: [0, 1, 2] })
...
gulp.task 'cloudfront-invalidation-next', ->
    ...
    execSync("aws cloudfront create-invalidation --distribution-id E9MTE3E9ESDLE --paths /CraftManager/#{env}/next/appcast.xml /CraftManager/#{env}/next/appcast-next.xml /CraftManager/#{env}/next/CraftManager.zip /CraftManager/#{env}/next/whats-new.html", { stdio: [0, 1, 2] })
...
gulp.task 'cloudfront-invalidation', ->
    ...
    env = config.env
    execSync("aws configure set preview.cloudfront true")
    execSync("aws cloudfront create-invalidation --distribution-id E9MTE3E9ESDLE --paths /CraftManager/#{env}/appcast.xml /CraftManager/#{env}/CraftManager.zip /CraftManager/#{env}/CraftInstaller.zip /CraftManager/#{env}/whats-new.html", { stdio: [0, 1, 2] })
...
gulp.task 'update-whats-new-page', ->
    console.log("S3 URL:    https://s3.amazonaws.com/#{config.amazonBucket}whats-new.html")
    console.log("Craft URL: #{config.craftUrl}whats-new.html")

    # Upload to s3 bucket
    execSync("aws s3 cp #{config.whatsNewPath} s3://#{config.amazonBucket}whats-new.html --acl public-read", { stdio: [0, 1, 2] })

    # Invalidate the cloudfront cache
    execSync("aws configure set preview.cloudfront true")
    execSync("aws cloudfront create-invalidation --distribution-id E9MTE3E9ESDLE --paths /CraftManager/#{config.env}/whats-new.html", { stdio: [0, 1, 2] })
    envSet('WHATS_NEW_URL', "#{config.craftUrl}whats-new.html")
...
gulp.task 'notarize-app', ->
    process.chdir(config.buildPath)
    try
        execSync("rm #{config.appName}.zip")

    execSync("zip -r -y #{config.appName}.zip #{config.appName}.app")
    
    console.log("Zipped app")

    if process.argv.indexOf("--nonotarization") >- 1
        console.log "No notarization"
        return  
    
    ITC_USERNAME = execSync("printf $ITC_USERNAME")
    ITC_PASSWORD = execSync("printf $ITC_PASSWORD")

    console.log("Request notarization")
    try 
        output = execSync("xcrun altool --output-format xml --notarize-app --primary-bundle-id 'com.invisionlabs.CraftManager' --username '#{ITC_USERNAME}' --password '#{ITC_PASSWORD}' --file #{config.appName}.zip")
        output = output.toString()
    catch ex
        console.log("xcrun altool ended in an error")
        output = ex.message    
    console.log("Request notarization complete")
    
    try
        jsonObj = plist.parse(output);
        message = jsonObj["success-message"]
        requestUUID = jsonObj["notarization-upload"]["RequestUUID"]
    catch error
    ...
    envSet('RequestUUID', "#{requestUUID}")
    console.log("RequestUUID: #{requestUUID}")

    count = 30
    validated = false
    for ii in [1...count]
        console.log("Wait for notarization... #{ii} of #{count}")
        sleep.sleep(10) # wait for approval
        try 
            output = execSync("xcrun altool --output-format xml --notarization-info #{requestUUID} --username '#{ITC_USERNAME}' --password '#{ITC_PASSWORD}'")
            output = output.toString()
        catch
        ...
        try
            jsonObj = plist.parse(output);
            status = jsonObj["notarization-info"]["Status"]
        catch error
            console.log(jsonObj)
            console.log("Status: invalid")
            console.log(error.message)
    ...
    if process.argv.indexOf("--nostapple") >- 1
        console.log "No stapple of notarization"
        return  

    if validated == false
        console.log("Failed validation... let us try to staple anyway")

    console.log("Staple and validate")
    try
        output = execSync("xcrun stapler staple '#{config.appName}.app'")
        output = execSync("xcrun stapler staple '#{config.appName}.app/Contents/Library/LoginItems/CraftManagerHelper.app'")
        output = output.toString()
    catch ex
        console.log(ex.message)
        throw ex

    if output.indexOf("The staple and validate action worked!") == -1
        console.log(output)
        throw "Notarization is not stappled"
        
    console.log("The staple and validate action worked!")
    #re zip
    console.log("Re zip the application")
    execSync("rm #{config.appName}.zip")
    execSync("zip -r -y #{config.appName}.zip #{config.appName}.app")
...
gulp.task 'go-live', (callback) ->
    runSequence(
        ...
        'test-update',
        'backup-s3-version',
        'backup-s3-plugins-on-retire',
        'backup-previous-version',
        'move-next-version',
        'backup-s3-version',
        'backup-s3-plugins-on-release',
...
copyFilesFromSourceToDest = (fromPath, destPath) ->
    ...
    execSync("aws s3 cp #{fromPath}whats-new.html #{destPath}whats-new.html --acl public-read", { stdio: [0, 1, 2] })
